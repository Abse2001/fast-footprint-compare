    ...
    for (const fixedCoordinate of fixedCoordinates) {
      const movingCoordinates = clusterCoordinates(
        regularPads
          .filter(
            (pad) =>
              Math.abs(pad[fixedAxis] - fixedCoordinate) <= tolerance,
          )
      ...
      differences.push(
        ...movingCoordinates
          ...
          .map(
            (coordinate, index) =>
              coordinate - movingCoordinates[index],
          )
...
const tryBuild = (footprinterString: string) => {
  try {
    return buildFootprinterPreview(footprinterString)
  } catch {
    return null
  }
}
...
const generateSeeds = (target: FootprintPreview, analysis: TargetAnalysis) => {
  ...
  if (analysis.topology === 'grid') {
    seeds.add(
      `bga${padCount}_grid${analysis.gridColumns}x${analysis.gridRows}`,
    )
  }
  ...
  if (padCount === 2 && analysis.platedHoleCount === 0) {
    ...
    )}_ph${formatMillimeters(
      median(padBounds.map((bound) => bound.height)),
    )}`
...
const optimizeSeed = (
  ...
  for (let iteration = 1; iteration <= OPTIMIZATION_STEPS; iteration += 1) {
    ...
    for (const parameter of activeParameters) {
      ...
      const firstMoment =
        0.9 * (firstMoments[parameter] ?? 0) + 0.1 * gradient
      const secondMoment =
      ...
      const stepScale = Math.max(
        analysis.heuristics[parameter] * 0.075,
        0.025,
      )
